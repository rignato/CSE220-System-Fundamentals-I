...
# 4($sp) = block_buffer (store ASCII characters)
	...
	blez $a2, bifid_encrypt.exit	# exit with -1 if period <= 0 
	...
	addi $sp, $sp, -8	# allocate stack space for 4 registers
	sw $s5, 0($sp)
	blt $s3, $s5, bifid_encrypt.period_smaller
	li $t0, 1
	li $s7, -1
	j bifid_encrypt.no_partial
	bifid_encrypt.period_smaller:
	div $s5, $s3
	mflo $s5			# number of complete blocks in $s5
	mfhi $s7			# size of partial block in $s7
	move $t0, $s5
	...
	lw $s5, 0($sp)		# load back strlen in $s5
	lw $s7, 4($sp)		# load total blocks created in $s7
	...
	# i = row, j = col
	bifid_encrypt.chars:
		beq $t4, $s5, bifid_encrypt.chars.end	# end loop if i = len(plaintext)
		lbu $t0, 0($s4)
		lbu $t1, 1($s4)
		mul $t0, $t0, $t2
		add $t0, $t0, $t1
...
# 4($sp) = block_buffer (store ASCII characters)
	...
	blez $a2, bifid_decrypt.exit	# exit with -1 if period <= 0
	...
	lw $s4, 0($sp)	# open index buffer in $s4
	...
	bifid_decrypt.indices:
		...
		addi $s1, $s1, 0		# curr_char++
		addi $s4, $s4, 2		# index_buffer++
		j bifid_decrypt.indices
	bifid_decrypt.indices.end:
	
	lw $s4, 0($sp)		# restore index_buffer ptr
	
	blt $s3, $s5, bifid_decrypt.period_smaller
	li $t0, 1
	li $s7, -1
	j bifid_decrypt.no_partial
	bifid_decrypt.period_smaller:
	div $s5, $s3
	mflo $s5			# number of complete blocks in $s5
	mfhi $s7			# size of partial block in $s7
	move $t0, $s5
	...
	bifid_decrypt.chars:
		beq $s6, $s5, bifid_decrypt.chars.end
		
		move $t0, $s3
		bgez $s7, bifid_decrypt.chars.period_smaller
		move $t0, $s5
		j bifid_decrypt.chars.continue
		bifid_decrypt.chars.period_smaller:
		blt $s6, $s3, bifid_decrypt.chars.continue
		div $s6, $s3
		mfhi $t0
		bnez $t0, bifid_decrypt.not_first_of_block
		add $s4, $s4, $s3
		add $s4, $s4, $s3
		bifid_decrypt.not_first_of_block:
		bifid_decrypt.chars.continue:
		lbu $t0, 0($s4)
		add $t1, $s4, $t2
